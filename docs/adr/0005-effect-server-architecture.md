...
- **The cross-namespace coupling smell (`gitlab/review-service.ts → JiraIssueService`) disappears.** `server/contexts/review/application/get-review-cards.ts` depends on `JiraGateway` and `GitlabGateway` Tags as peers; neither gateway depends on the other.
