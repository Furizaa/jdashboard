...
## Phase-1 follow-up — Schema bodies, transport-error split, concurrency parity (2026-05-10)

After Phase 1 landed, an audit pass picked up three places where the as-merged code had drifted from this ADR's prose: HTTP body decoding was hand-rolled rather than Schema-validated, transport-class failures were folded into `Rejected` rather than carried as a peer tag, and the inner per-MR fan-out kept the documented "5 at the application-service level" honest only by accident. None of these are corrections to the original design — each is a chosen point on the policy menus this ADR set up, exercised now that real callers exist. Slices 75–82 are the implementation paper trail.

The concrete changes:

- **Schema-validated HTTP bodies via `HttpClientResponse.schemaBodyJson`.** Both gateway adapters now decode JSON bodies through `effect/Schema` structs (`RawSearchResponseSchema`, `RawDetailedIssueSchema`, `JiraUserResponseSchema`, the GitLab equivalents). Decode failures fail with `TransportError`, distinct from a 4xx `Rejected`. The libraries-table row's "HTTP response shapes" rule is now load-bearing rather than aspirational.
- **`TransportError` introduced as a peer of `Rejected`.** `Rejected` keeps its narrow meaning — the upstream answered with a 4xx body. Network failures, request-encoding failures, and response-decode failures get their own `TransportError` tag carrying a `message`. Per-context error unions choose: writes (`quickCreate`, `performTransition`) surface `Unauthorized | Rejected | TransportError` so a transport blip during a mutation reaches the user as a tagged failure; read paths (`loadIssue`, `loadTransitions`, `loadBoard`, `loadMyself`, `loadMyEpics`, `loadMrStatuses`, `loadReviewCards`) retain `Unauthorized` (and `NotFound` where it's load-bearing) and demote the rest to defects via the new `dieOn` combinator. The taxonomy is documented in `CONTEXT-MAP.md`'s wire-error-taxonomy section.
- **`mr-fanout`'s inner concurrency capped at 1.** The per-MR sub-call fan-out (`getMr`, `getMrDiscussions`, `getMrApprovals`, `getMrReviewers`) now runs sequentially; the only in-flight budget is the `concurrency: 5` outer fan-out at the application-service level. The documented number and the runtime number are the same.
- **`Schedule.compose(Schedule.recurs(N))` retry-cap pattern annotated.** A one-line comment at `src/server/runtime/app-layer.ts` records that `Schedule.compose` _intersects_ the two schedules: keep the exponential cadence; stop after `RETRY_ATTEMPTS` retries. This is the policy-menu's chosen point made readable at the call site.
- **`runWire` helper at the server-function boundary.** `src/server/server-functions/run-wire.ts` lifts the `appRuntime.runPromise(toWire(program, ErrorSchema))` recipe out of every handler and demotes the `InternalError` envelope branch to a thrown `Error` so react-query's `isError` flag flips at the call site. Tagged failures stay in the wire shape unchanged.
- **`dieOn` combinator for tag demotion.** `src/server/lib/die-on.ts` is the typed alternative to `Effect.catchTags({ ...: Effect.die })`. Tag literals are constrained against the input effect's error union — `dieOn('NotFOund')` is a compile error, not a silent no-op. This is what makes the per-context demotion lines (`dieOn('NotFound', 'Rejected', 'TransportError')`) self-documenting.
- **`assertIssueKey` validator at the server-function input boundary.** `src/server/lib/jql.ts` exposes `assertIssueKey` and `quoteJqlString`; the Detail server functions' `inputValidator`s call `assertIssueKey` so an attacker-controlled `key` cannot break out of a JQL fragment. Server-built JQL elsewhere (Board's `searchIssues`, Capture's epic JQL, Detail's parent search, Review's bulk lookup) goes through `quoteJqlString`.

Cross-link to the slice issues for the full implementation history: [75 — JQL safety helper + `assertIssueKey`](../../.agents/issues/75-effect-server-jql-safety.md), [76 — `dieOn` combinator](../../.agents/issues/76-effect-server-die-on-combinator.md), [77 — `runWire` helper](../../.agents/issues/77-effect-server-run-wire-helper.md), [78 — polymorphic `notImpl` in fake gateways](../../.agents/issues/78-effect-server-fake-gateway-not-impl-poly.md), [79 — `getGitlabUser` wire-shape unification](../../.agents/issues/79-effect-server-gitlab-user-wire-shape.md), [80 — `TransportError` + concurrency cap + `Schedule.compose` comment](../../.agents/issues/80-effect-server-transport-error-and-concurrency.md), [81 — Schema-validated HTTP bodies (Jira)](../../.agents/issues/81-effect-server-schema-bodies-jira.md), [82 — Schema-validated HTTP bodies (GitLab)](../../.agents/issues/82-effect-server-schema-bodies-gitlab.md).
